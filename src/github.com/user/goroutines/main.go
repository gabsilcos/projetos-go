...
//execução concorrente, fragmentação multicore
...
import (
	...
	"github.com/user/goroutines/model"
)

var orquestrador sync.WaitGroup //administrador de passo de execução

func main() {
	orquestrador.Add(2) //vai gerenciar duas tarefas
	log.Println("****** Começando...")
	go traduzirParaJSON("saopaulo") //manda executar e dane-se
	go traduzirParaJSON("riodejaneiro")
	orquestrador.Wait() //manda aguardar a execução
	log.Println("****** Acabei! ******")
}
...
func traduzirParaJSON(nomeArquivo string) {
	...
	orquestrador.Done() //avisa ao administrador que terminou de executar
}
