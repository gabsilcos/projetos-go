...
import (
	...
	"github.com/user/banco_sql/model"
	"github.com/user/banco_sql/repo"
)
...
func Local(w http.ResponseWriter, r *http.Request) {
	...
	codigoTelefone, err := strconv.Atoi(r.URL.Path[7:]) //retorna o conteúdo digitado no request
	//                                                  //a partir do sétimo caracter
	if err != nil {
	...
	linha, err := db.Queryx(sql, codigoTelefone) //realiza a pesquisa
	if err != nil {
	...
	for linha.Next() { //se não retornar só um resultado
	...
	//aqui termina o processo de leitura

	//insert no banco em si:
	//no caso um seguidor de operações
